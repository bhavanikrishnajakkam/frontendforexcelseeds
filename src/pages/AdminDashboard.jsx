...
export default function AdminDashboard() {
  ...
  const [formData, setFormData] = useState({
    cropName: '', packedVariety: '', packedLotNumber: '',
    dateOfTesting: '', dateOfPackaging: '', dateOfExpiry: '',
    mrp: '', unitSalePrice: '', netQty: '',
    packedAt: 'Nandeeshwara Seeds - Huzurabad.', plantAddress: 'Excel Agri Research Pvt. Ltd.\nC/o. Nandeeshwara Seeds,\nH.No: 4-191, Peddapapaipally,\nHUZURABAD.', producedBy: 'EXCEL AGRI RESEARCH PVT.LTD',
    quantity: 10
  });
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { alert("Error generating batch."); }
        ...
        <form onSubmit={handleSubmit} style={styles.formCard}>
          <div style={styles.grid}>
            ...
            <div style={styles.column}>
              ...
              <input style={styles.input} name="cropName" placeholder="Crop Name" onChange={handleChange} required />
              <input style={styles.input} name="packedVariety" placeholder="Packed Variety" onChange={handleChange} required />
              ...
              <input style={styles.input} type="date" name="dateOfPackaging" onChange={handleChange} required />
            ...
            <div style={styles.column}>
              ...
              <input style={styles.input} name="packedAt" placeholder="Packed At (Unit Name)" value={formData.packedAt} onChange={handleChange} required />
              <textarea style={{ ...styles.input, height: '80px' }} name="plantAddress" value={formData.plantAddress} placeholder="Full Plant Address" onChange={handleChange} required />
...
const styles = {
  pageWrapper: { backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' },
  ...
  logoutBtn: { background: 'none', border: '1px solid #fff', color: '#fff', padding: '5px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' },
  container: { maxWidth: '900px', margin: '0 auto', padding: '20px' },
  header: { textAlign: 'center', marginBottom: '30px' },
  logo: { width: '70px', height: '70px', borderRadius: '50%', marginBottom: '10px' },
  brandTitle: { color: '#064e3b', fontSize: '28px', fontWeight: '800' },
  formCard: { backgroundColor: '#fff', padding: '30px', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', border: '1px solid #e2e8f0' },
  grid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' },
  sectionTitle: { fontSize: '14px', color: '#065f46', fontWeight: '700', borderBottom: '1px solid #eee', paddingBottom: '5px', marginBottom: '15px', textTransform: 'uppercase' },
  input: { width: '100%', padding: '10px', marginBottom: '12px', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box', outline: 'none' },
  label: { fontSize: '11px', fontWeight: '700', color: '#64748b', marginBottom: '4px', display: 'block' },
  button: { width: '100%', padding: '14px', backgroundColor: '#059669', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '16px', fontWeight: '700' },
